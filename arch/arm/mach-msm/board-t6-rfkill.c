...
struct pm8xxx_gpio_init t6_bt_pmic_gpio[] = {
...
static uint32_t t6_GPIO_bt_on_table[] = {
...
static uint32_t t6_GPIO_bt_off_table[] = {
...
static void t6_GPIO_config_bt_on(void)
{
	printk(KERN_INFO "[BT]== R ON ==\n");

	
	config_bt_table(t6_GPIO_bt_on_table,
				ARRAY_SIZE(t6_GPIO_bt_on_table));
	mdelay(2);
...
static void t6_GPIO_config_bt_off(void)
{
	if (system_rev < XC) {
	...
	config_bt_table(t6_GPIO_bt_off_table,
				ARRAY_SIZE(t6_GPIO_bt_off_table));
	mdelay(2);
...
{
	if (!blocked)
		t6_GPIO_config_bt_on();
	else
		t6_GPIO_config_bt_off();

	return 0;
}

static struct rfkill_ops t6_rfkill_ops = {
	.set_block = bluetooth_set_power,
};

static int t6_rfkill_probe(struct platform_device *pdev)
{
	int rc = 0;
	...
	for( i = 0; i < ARRAY_SIZE(t6_bt_pmic_gpio); i++) {
		rc = pm8xxx_gpio_config(t6_bt_pmic_gpio[i].gpio,
					&t6_bt_pmic_gpio[i].config);
		if (rc)
			pr_info("[bt] %s: Config ERROR: GPIO=%u, rc=%d\n",
				__func__, t6_bt_pmic_gpio[i].gpio, rc);
	}
...
static int t6_rfkill_remove(struct platform_device *dev)
{
...
static struct platform_driver t6_rfkill_driver = {
	.probe = t6_rfkill_probe,
	.remove = t6_rfkill_remove,
	.driver = {
		.name = "t6_rfkill",
		.owner = THIS_MODULE,
	},
...
static int __init t6_rfkill_init(void)
{
	return platform_driver_register(&t6_rfkill_driver);
}

static void __exit t6_rfkill_exit(void)
{
	platform_driver_unregister(&t6_rfkill_driver);
}

module_init(t6_rfkill_init);
module_exit(t6_rfkill_exit);
MODULE_DESCRIPTION("t6 rfkill");
