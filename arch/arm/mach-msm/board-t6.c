...
static struct platform_device t6_rfkill = {
	.name = "t6_rfkill",
	.id = -1,
};
#endif
...
static int t6_g_sensor_power_LPM(int on)
{
	int rc = 0;
...
static struct bma250_platform_data gsensor_bma250_platform_data = {
	...
	.negate_y = 1,
	.negate_z = 1,
	.power_LPM = t6_g_sensor_power_LPM,
};

static int t6_compass_power_LPM(int on)
{
	int rc = 0;
...
static struct akm8963_platform_data compass_platform_data = {
	...
	.gpio_RST = 0,
	.power_LPM = t6_compass_power_LPM,
};

static int t6_gyro_power_LPM(int on)
{
	int rc = 0;
...
static struct msm_i2c_platform_data t6_i2c_qup_gsbi1_pdata = {
	.clk_freq = 400000,
...
static void __init t6_i2c_init(void)
{
	...
	apq8064_device_qup_i2c_gsbi1.dev.platform_data =
					&t6_i2c_qup_gsbi1_pdata;
	gsbi_mem = ioremap_nocache(MSM_GSBI1_PHYS, 4);
	writel_relaxed(GSBI_DUAL_MODE_CODE, gsbi_mem);
	/* Ensure protocol code is written before proceeding */
	wmb();
	iounmap(gsbi_mem);
	t6_i2c_qup_gsbi1_pdata.use_gsbi_shared_mode = 1;
...
static void __init register_i2c_devices(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(t6_i2c_devices); ++i) {
		if (t6_i2c_devices[i].machs & mach_mask) {
			if (system_rev != XA) {
				if (!strcmp(t6_i2c_devices[i].info->type, "smb349")) {
...
static void __init t6_smb349_mpp_init(void)
{
...
static void __init t6_fixup(struct tag *tags, char **cmdline, struct meminfo *mi)
{
	...
	printk(KERN_INFO "t6_fixup:skuid=0x%x\n", skuid);
}
